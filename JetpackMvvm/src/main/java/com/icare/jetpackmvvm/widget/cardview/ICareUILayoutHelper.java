package com.icare.jetpackmvvm.widget.cardview;
...
public class ICareUILayoutHelper implements ICareLayout {
    ...
    public ICareUILayoutHelper(Context context, AttributeSet attrs, int defAttr, View owner) {
        this(context, attrs, defAttr, 0, owner);
    }

    public ICareUILayoutHelper(Context context, AttributeSet attrs, int defAttr, int defStyleRes, View owner) {
        mContext = context;
        mOwner = new WeakReference<>(owner);
        mBottomDividerColor = mTopDividerColor =
                ContextCompat.getColor(context, R.color.icare_config_color_separator);
        mMode = new PorterDuffXfermode(PorterDuff.Mode.DST_OUT);
        mClipPaint = new Paint();
        mClipPaint.setAntiAlias(true);
        mShadowAlpha = ICareUIResHelper.getAttrFloatValue(context, R.attr.icare_general_shadow_alpha);
        ...
        if (null != attrs || defAttr != 0 || defStyleRes != 0) {
            ...
            for (int i = 0; i < count; ++i) {
                ...
                } else if (index == R.styleable.ICareLayout_icare_outlineInsetLeft) {
                    mOutlineInsetLeft = ta.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.ICareLayout_icare_outlineInsetRight) {
                    mOutlineInsetRight = ta.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.ICareLayout_icare_outlineInsetTop) {
                    mOutlineInsetTop = ta.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.ICareLayout_icare_outlineInsetBottom) {
                    mOutlineInsetBottom = ta.getDimensionPixelSize(index, 0);
                } else if (index == R.styleable.ICareLayout_icare_outlineExcludePadding) {
