...
class IRecyclerView : RecyclerView {
    ...
    var actualAdapter: RecyclerView.Adapter<*>? = null
        private set
    private var mIAdapter: IAdapter? = null
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        mIAdapter = IAdapter()
        val a = context.obtainStyledAttributes(attrs, R.styleable.IRecyclerView, 0, 0)
        ...
        try {
            ...
            val value = a.getInteger(R.styleable.IRecyclerView_i_demo_layout_manager_type, 0)
            when (value) {
        ...
        mIAdapter!!.setShimmerAngle(mShimmerAngle)
        mIAdapter!!.setShimmerColor(mShimmerColor)
        mIAdapter!!.setShimmerMaskWidth(mShimmerMaskWidth)
        if (mShimmerItemBackground != null)
            mIAdapter!!.setShimmerItemBackground(mShimmerItemBackground)
        mIAdapter!!.setShimmerDuration(mShimmerDuration)
        mIAdapter!!.setAnimationReversed(isAnimationReversed)
    ...
    fun setDemoChildCount(count: Int) {
        mIAdapter!!.setMinItemCount(count)
    }
    ...
    fun setDemoShimmerDuration(duration: Int) {
        mIAdapter!!.setShimmerDuration(duration)
    }
    ...
    fun setDemoShimmerMaskWidth(maskWidth: Float) {
        mIAdapter!!.setShimmerMaskWidth(maskWidth)
    }
    ...
    fun setDemoLayoutReference(mLayoutReference: Int) {
        ...
        mIAdapter!!.setLayoutReference(layoutReference)
    }
